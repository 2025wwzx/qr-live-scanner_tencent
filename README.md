...
`tencent-status` 在确认账号已保存且已授权后会验证账号索引，不显示账号 ID 或凭证。
也可以使用“账号管理” -> “检查账号索引”显式检查当前 provider 的本地账号索引；
GUI 状态栏只显示可用数量、是否重建索引和清理陈旧索引数量，不显示账号 ID 或凭证。
选中账号后可以使用“账号管理” -> “检查选中账号状态”验证登录态、授权标记和本地索引，
状态栏只显示概要，不显示账号 ID 或凭证。
...
本地账号自检也会做账号索引验证，确保自检账号能被“导入全部已保存账号”发现。
清理本地自检也会验证账号索引已清理，防止 GUI 账号表和本地索引状态脱节。
...
导入已保存账号弹窗和本地账号自检弹窗。
带 `--mock-uid` 时还会生成 `main-window-account-status.png`，展示“检查选中账号状态”
后的状态栏概要。
...
UID、二维码 payload、敏感 `app_id` 值、敏感 path 片段或任何已签名 URL 写进配置文件。
...
Supported real-shape account protocol modes:
...
app_id = "your-verified-qq-app-id"
...
For QQ Connect, set `QR_LIVE_SCANNER_TENCENT_QQ_APP_SECRET` in the local
environment before running `tencent-login`. For WeChat, set
